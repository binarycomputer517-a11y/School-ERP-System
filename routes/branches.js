 ...
 * @version 2.9.6 (Final Production - English Version)
 * Fixes: Route redundancy, error handling logic, and security gating.
 */

const express = require('express');
const router = express.Router();
const bcrypt = require('bcryptjs'); // Using bcryptjs for environment consistency
const { pool } = require('../database');
const { authenticateToken, authorize } = require('../authMiddleware');
const { sendManagerProvisionEmail } = require('../utils/mailer'); 
...
// =========================================================
// 2. FULL PROVISION (Atomic Transaction: Branch + Admin User + Email)
// =========================================================
    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            const userRes = await client.query(userQuery, [
                ...
                branch.email, // Branch email used for user communication
            ...
            // 🎯 FEATURE: Automated Welcome Email
            // Sent after COMMIT to ensure data integrity before notification
            try {
                await sendManagerProvisionEmail(
                    newManager.email, 
                    newManager.full_name, 
                    user.password, // Send raw password for initial login
                    branch.branch_code
                );
            } catch (mailErr) {
                console.error('Mail Delivery Error:', mailErr.message);
                // We don't rollback here as the DB transaction is already successful
            }
...
router.delete('/:id', authenticateToken, authorize(AUTH_ROLES), async (req, res) => {
    ...
    try {
        ...
        if (error.code === '23503') {
            return res.status(409).json({ message: 'Branch has active records (Students/Staff) and cannot be deleted.' });
        }
...
// =========================================================
// 5. GET SINGLE BRANCH (Security Gated Lookup)
// =========================================================
router.get('/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // If not super admin, check if user belongs to this specific branch
        ...
        console.error('Fetch Branch Detail Error:', error);
        res.status(500).json({ message: 'Internal Server Error' });
    }
});
