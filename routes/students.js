...
// ---------------------------------------------------------
// 4. PUBLIC LOOKUP: No Token Required
// ---------------------------------------------------------
router.get('/public/lookup/:enrollmentId', async (req, res) => {
    const { enrollmentId } = req.params;
    try {
        const query = `
            SELECT s.first_name, s.last_name, s.enrollment_no, s.status, c.course_name, s.profile_image_path
            FROM students s
            LEFT JOIN courses c ON s.course_id = c.id
            WHERE s.enrollment_no = $1 OR s.admission_id = $1 LIMIT 1
        `;
