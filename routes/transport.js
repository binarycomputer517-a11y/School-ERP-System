...
// =================================================================
// 27. STUDENT: GET BUS LOCATION (Updated with Assignment Check)
// =================================================================
router.get('/student/:sid/bus-location', authenticateToken, async (req, res) => {
    ...
    try {
        const query = `
            ...
            FROM student_transport_assignments sta
            JOIN transport_vehicles v ON sta.vehicle_id = v.id
            WHERE sta.student_id = $1::uuid AND sta.is_active = TRUE;
        `;

        const { rows } = await pool.query(query, [sid]);

        // Check if student has an active bus assignment
        if (rows.length === 0) {
        ...
        // Prepare base response for assigned students
        const response = {
            assigned: true,
        ...
        // Split coordinates only if they exist
        if (vehicle.current_coords && vehicle.current_coords.includes(',')) {
            const [lat, lng] = vehicle.current_coords.split(',');
            response.latitude = lat.trim();
            response.longitude = lng.trim();
        }
        ...
        res.status(500).json({ 
            ...
            detail: "Failed to fetch transport data." 
        });
    }
});
